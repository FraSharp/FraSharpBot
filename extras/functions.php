...
function isAllowed($userId) {
    $perm = json_decode(file_get_contents("extras/perm.json"));
...
function sendUserPic($chatId, $userId, $caption = NULL, $sendAsDocument = false) {
    ...
    if ($sendAsDocument == false) {
        ...
        if ($sendAsDocument == true) {
            ...
            return shell_exec("curl -v -F document=@downloads/pic.png https://api.telegram.org/bot" . $TOKEN . "/sendDocument?chat_id=" . $chatId . "&caption=" . $infos);
        }
    }
}


function importDatabase(string $filePath) {
    $conn = new mysqli("host", "user", "password", "frasharpbot");
    $import = false;
...
function warnMember($chatid, $userid) {
    ...
    $a = $checkIfUserExistsQuery->fetch_assoc();
    if ($a == null or $a == false or $a == "") {
